...
**Note**: The microservice loads a very large dataset at startup to show the power of the paging, sorting, and search capabilities in Spring Data. Free database service tiers on public Cloud Foundry services often limit the size of the database you can use and the number of records you can load at startup. You will likely need to reduce the size of the link:cities-service/src/main/resources/import.sql[dataset] when deploying to a public Cloud Foundry service with a free database tier.
